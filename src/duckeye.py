...
import os
from src.config import MODELS_PATH, SEGMENTATION_MODEL, BM_REGRESSOR_MODEL, PREPROCESSED_PATH, OUTPUT_PATH ,DEFAULT_OUTPUT_SIGNATURE
import src.preprocessing.preprocessing as prep

class DuckEYE():

    def __init__(self, input_dir:Path, device:str):
    ...
    def observe(self, run_name:str):

        signature = DEFAULT_OUTPUT_SIGNATURE.replace('XX',str(run_name))
        output_dir = OUTPUT_PATH / signature
        output_dir.mkdir(parents=True,exist_ok=True)

        print('starting observation')

        #prep.resize(self.input_dir)

        calculated_density_list = []
        segmented_density_list = []
        predicted_biomass_list = []

        files = []
        for file in os.listdir(PREPROCESSED_PATH):
            full_path = PREPROCESSED_PATH / file
            files.append(file)
            ...
            calculated_density_list.append(calculated_density)
            segmented_density_list.append(segmented_density)
            predicted_biomass_list.append(predicted_biomass_density)

            plt.figure(figsize=(12, 5))
            plt.subplot(1, 3, 1)
            plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
            plt.title(f"Predicted biomass: {predicted_biomass_density:.3f} g/m^2")
            plt.axis('off')
            ...
            plt.savefig(output_dir / f'{file}.png')
            plt.close()

        df = pd.DataFrame(
        {
            'img':files,
            'calculated_density':calculated_density_list,
            'predicted_density':segmented_density_list,
            'predicted_biomass':predicted_biomass_list
        }
        )
